...
// GETTING DATA FROM SQL BREEDS TABLE TO SINCE COWS TABLE DEPENDS ON
...
// GETTING DATA FROM SQL COWS TABLE
app.get('/cows', async(req, res) => {
  try {
    const data = await client.query(`SELECT cows.id,
    cows.sex,
    cows.number_horns,
    cows.milk,
    breeds.breed_type AS breed
    FROM cows INNER JOIN breeds
    ON cows.breed_id = breeds.id
    ORDER BY cows.id`);
    res.json(data.rows);
  } catch(e) {
    res.status(500).json({ error: e.message });
